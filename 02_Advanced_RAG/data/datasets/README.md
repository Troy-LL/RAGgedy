...
2. Optionally add `datasets/<your_id>/questions.json` with entries compatible with `evaluation/eval_advanced.py`.
3. Run with `RAGGEDY_DATASET=<your_id>` so ingest and query use the new paths and separate stores under `.chroma/<your_id>/` and `.bm25_index/<your_id>/`.
